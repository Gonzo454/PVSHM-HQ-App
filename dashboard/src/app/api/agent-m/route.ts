...
function getSystemPrompt(): string {
  return `You are PV Assistant, a financial intelligence assistant for Park Vista Senior Housing Management. You have access to live AppFolio property management data through tool calls.

Park Vista manages 8 senior living communities across WI, IA, and IL offering independent living, assisted living, and memory care:
- Noel Manor (WI)
...
- Income statements (P&L) for any date range or community
...
- Community-level P&L for individual communities
- Account totals (community balances)
...
3. Highlight anything concerning (e.g., high delinquency, negative cash flow, low occupancy)
...
7. Use "community" or "communities" instead of "property" or "properties" when referring to Park Vista locations
...
const tools: OpenAI.Chat.Completions.ChatCompletionTool[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description: "Get P&L for a specific community by name or slug. Returns income, expenses, net income for that community.",
      parameters: {
        type: "object",
        properties: {
          property: { type: "string", description: "Community name or slug (e.g., 'Noel Manor' or 'noel-manor')" },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Get outstanding receivables grouped by resident with aging buckets (current, 31-60, 61-90, 90+ days). Shows who owes money and how overdue.",
  ...
  },
  {
    ...
    function: {
      name: "get_rent_roll",
      description: "Get full rent roll showing all units, residents, occupancy status, market/actual rents, lease end dates, and balances.",
      parameters: { type: "object", properties: {} },
    },
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Get community-level account balance totals. Shows net amounts per community.",
      parameters: { type: "object", properties: {} },
    },
  },
];
  ...
  try {
    ...
    switch (name) {
      ...
      case "get_property_pnl":
        data = await callInternalApi(baseUrl, "/api/community-pnl", { community: args.property, ...args });
        break;
      case "get_cash_flow":
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("PV Assistant error:", err);
    return Response.json(
      { error: err instanceof Error ? err.message : "PV Assistant encountered an error" },
      { status: 500 }
    );
  }
}
