...
export default function KPIDashboardPage() {
  ...
  const { portfolio: p, communities } = data;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          {p.propertyCount} Properties · {data.period.label} {data.period.from} – {data.period.to} · Operating Metrics
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <p className="text-xs text-gray-500 mt-0.5">Sorted by revenue · {data.period.label} actuals</p>
        </div>
              ...
              {communities.map((c) => (
                <tr key={c.slug} className="hover:bg-gray-50 dark:hover:bg-gray-750">
                  <td className="px-4 py-3">
                    <Link href={`/communities/${c.slug}`} className="text-[#2E7D6F] hover:underline font-medium">
                      {c.name}
                    </Link>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <p className="text-xs text-gray-500 mt-0.5">From current rent roll · Target: 90%+</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-700 text-xs uppercase">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <p className="text-xs text-gray-500 mt-0.5">DSCR = NOI ÷ Debt Service · OER = Expenses ÷ Revenue · Labor target: 45–50%</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-700 text-xs uppercase">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {communities.filter((c) => c.revenue > 0).map((c) => (
                <tr key={c.slug} className="hover:bg-gray-50 dark:hover:bg-gray-750">
                  <td className="px-4 py-3">
                    <Link href={`/communities/${c.slug}`} className="text-[#2E7D6F] hover:underline font-medium">
                      {c.name}
